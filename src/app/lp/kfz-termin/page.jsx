...
export default function KfzTermin() {
  const mob = useIsMobile();
  const [callbackForm, setCallbackForm] = useState({ name: "", telefon: "", betrieb: "" });
    ...
    try {
      await fetch("/api/leadmagnet-capture", {
        ...
        body: JSON.stringify({
          ...
          company: callbackForm.betrieb,
          email: `rueckruf-${Date.now()}@kfz-lead.talentsuite.io`,
      ...
      <div style={{ background: `linear-gradient(135deg, ${DARK} 0%, ${BRAND} 100%)`, padding: mob ? "48px 16px 56px" : "80px 24px 90px", textAlign: "center" }}>
        <div style={containerStyle}>
          ...
          <h1 data-pf="white" style={{ color: "#ffffff", fontSize: mob ? 28 : 40, fontWeight: 800, lineHeight: 1.15, margin: "0 0 16px" }}>
            ...
            <span data-pf="accent" style={{ color: ACCENT }}>für KFZ-Fachkräfte</span>
          </h1>
          <p data-pf="white70" style={{ color: "rgba(255,255,255,0.7)", fontSize: mob ? 16 : 18, lineHeight: 1.6, maxWidth: 520, margin: "0 auto 32px" }}>
            In 15 Minuten zeigen wir Ihnen exakt, wie viele KFZ-Fachkräfte Sie in den nächsten 30 Tagen gewinnen können.
          </p>
          <a
            href={CALENDAR_URL}
            target="_blank"
      ...
      <div style={{ marginTop: mob ? 40 : 60, marginBottom: mob ? 40 : 60 }}>
        <div style={containerStyle}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {[
              { icon: "📊", title: "Individuelle Standort-Analyse", desc: "Wir analysieren, wie viele KFZ-Fachkräfte in Ihrem Umkreis über Social Media erreichbar sind." },
              ...
              { icon: "🎯", title: "Strategie-Empfehlung", desc: "Maßgeschneidert für Ihren Betrieb, Ihre Region und Ihre offenen Stellen." },
            ].map((b, i) => (
        ...
        <div style={containerStyle}>
          <div style={{ display: "grid", gridTemplateColumns: mob ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
            {[
              ...
              { value: "42+", label: "Bewerbungen/Monat" },
              { value: "195€", label: "Ø pro Einstellung" },
          ...
          <div style={{ background: "#ffffff", borderRadius: 14, padding: mob ? "20px 18px" : "24px 24px", boxShadow: "0 4px 16px rgba(0,0,0,0.06)", borderLeft: `3px solid ${ACCENT}` }}>
            <p style={{ color: BRAND, fontSize: 15, fontStyle: "italic", lineHeight: 1.7, margin: "0 0 8px" }}>
              „42 Bewerbungen in 5 Wochen – komplett ohne StepStone oder Indeed. Wir haben inzwischen alle Stellen besetzt und unsere Werkstatt läuft wieder auf Volllast."
            </p>
            <p data-pf="gray" style={{ color: "#64748B", fontSize: 13, margin: 0 }}>– Geschäftsführer, Autohaus in NRW</p>
          </div>
        </div>
      </div>
          ...
          <div style={{ background: "#ffffff", borderRadius: 20, padding: mob ? "28px 20px" : "36px 32px", boxShadow: "0 4px 30px rgba(0,0,0,0.08)", textAlign: "center" }}>
            ...
            {callbackSubmitted ? (
              ...
              <form onSubmit={handleCallback} style={{ maxWidth: 400, margin: "0 auto" }}>
                ...
                <input type="text" value={callbackForm.betrieb} onChange={(e) => setCallbackForm({ ...callbackForm, betrieb: e.target.value })} placeholder="Werkstatt / Autohaus (optional)" style={{ ...inputStyle, marginBottom: 16 }} />
